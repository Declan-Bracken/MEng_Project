...
def display_aggrid(df, key):
    ...
    return selected_columns, pd.DataFrame(updated_df)
...
def information_extraction():
    ...
    if uploaded_file:
        ...
        if st.button("Query Mistral"):
            query_mistral(selected_header_text if headers else None, selected_table_text, mistral_model_path)
        ...
        if 'final_df' in st.session_state and st.session_state.final_df is not None:
            ...
            _, updated_df = display_aggrid(st.session_state.final_df, key='mistral_table')
...
def candidate_strength_assessment():
