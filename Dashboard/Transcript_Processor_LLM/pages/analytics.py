...
def app():
    ...
    # Check if 'final_df' is in session state
    if st.session_state.final_df is not None and len(st.session_state.final_df) > 0:
        df = st.session_state.final_df
        st.write("DataFrame available in session state:")
    ...
    if st.session_state.uploaded_df is not None and df is None:
        try:
    ...
    if df is not None:
        ...
        if st.session_state.result_df is not None:
            st.download_button("Download Result as CSV", st.session_state.result_df.to_csv(index=False), "dataframe_courses_classified.csv", "text/csv")
